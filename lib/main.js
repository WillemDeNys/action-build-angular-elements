...
async function run() {
    try {
        ...
        function buildConfig(element, project) {
            const config = fs.readFileSync('./build_scripts/rollup.config.js', {encoding:'utf8', flag:'r'});
            let new_config = config.replaceAll('${project}', project).replaceAll('${element}', element);
            fs.writeFileSync('./build_scripts/config.js', new_config);
        }

        function buildSingleElement(element, project) {

            if (fs.existsSync(`${root_folder}/${project}/${element}/src/${element}.lit.ts`)) {
                buildConfig(element, project);
                exec(`npx rollup -c ./build_scripts/config.js`,
                    (err, stdout, stderr) => {
                        if (err) throw err;
                        else {
                            console.log(stdout);
                            console.log("Finished building: " + element + " and moved output to " + output_folder);
                            console.log(element + " from " + project + " is successfully build and ready to be published.")
                        }
                    });
            }
            else {
                exec(`npx ng build --configuration production --project ${element} --output-hashing none`,
                    (err) => {
                        if (err) throw err;
                        else console.log("Finished building: " + element + " concatenating and moving output to " + output_folder);
                        concatenate(element, project);
                    });
            }
        }
        ...
        async function concatenate(elementName, projectName) {
            ...
            try {
                ...
                await fse.copyFile(root_folder + "/" + projectName + "/" + elementName + "/package.json", output_folder + "/" + elementName + "/package.json");
