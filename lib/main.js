...
async function run() {
    try {
        ...
        function buildSingleElement(element, project) {
            if (fs.existsSync(`${root_folder}/${project}/${element}/src/${element}.js`)) {
                const code = fs.readFileSync(`${root_folder}/${project}/${element}/src/${element}.js`, "utf8");
                const uglyCode = uglify.minify(code).code;
                fse.ensureDirSync(output_folder + "/" + element);
                fs.writeFileSync(`${output_folder}/${element}/${element}.js`, uglyCode);
